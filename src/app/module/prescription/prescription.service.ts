...
import { ICreatedPrescriptionPayload } from "./prescription.interface";

const createPrescription = async (payload: ICreatedPrescriptionPayload, user: RequestUser) => {
    ...
    if (appointment.status !== AppointmentStatus.COMPLETED) {
        throw new AppError(httpStatus.BAD_REQUEST, "Prescription can only be written for a completed appointment.");
    }
    ...
    const uploadResult = await new Promise<UploadApiResponse>((resolve, reject) => {
        cloudinary.uploader.upload_stream(
            ...
            (error, result) => {
                ...
                if (!result) {
                    return reject(new AppError(httpStatus.INTERNAL_SERVER_ERROR, "Failed to upload prescription PDF from Cloudinary."));
                }
                resolve(result);
            },
        ).end(pdfBuffer);
